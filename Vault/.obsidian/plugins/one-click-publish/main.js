...
const EMBEDDED_TARGET_PLUGIN_VERSION = "0.4.1";
...
const COPY = {
  ...
  workerVersionChecking: "Checking Worker version...",
  workerVersionCurrent: "Worker is up to date.",
...
const COPY_ZH = {
  ...
  workerVersionChecking: "正在检查 Worker 版本……",
  workerVersionCurrent: "Worker 已是当前版本。",
...
function workerVersionUiState(settings) {
  if (!settings?.deploymentWorkerUrl || !settings?.selfPublishToken) return "not_applicable";
  const status = normalizeWorkerVersionStatus(settings.workerVersionStatus);
  const version = String(settings.workerVersion || "").trim();
  if (status === "current" && version === EMBEDDED_TARGET_PLUGIN_VERSION) return "current";
  if (status === "unknown" && !version && !Number(settings.workerVersionCheckedAt || 0)) return "checking";
  return "needs_update";
}

function workerVersionNeedsUpdate(settings) {
  return workerVersionUiState(settings) === "needs_update";
}
...
class SharePublisherSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    const workerVersionState = personalSelected ? workerVersionUiState(this.plugin.settings) : "not_applicable";
    const showWorkerUpdateAction = workerVersionNeedsUpdate(this.plugin.settings);
    const workerVersionStatusText = workerVersionState === "checking"
      ? copy.workerVersionChecking
      : workerVersionState === "current"
        ? copy.workerVersionCurrent
        : "";
    ...
    const cloudflareSetting = new Setting(containerEl)
      ...
      .setDesc(`${copy.selfCloudflareDescription} ${copy.deploymentDescription} ${!hasPersonal && !isDesktopEnvironment() ? copy.desktopDeploymentOnly : ""} ${personalSelected ? copy.selectedMode : copy.notSelectedMode} ${workerVersionStatusText ? ` ${workerVersionStatusText}` : ""} ${hasPersonal ? copy.disconnectCloudflareDescription : ""} ${this.disconnectPending ? copy.disconnectConfirm : ""}${session?.active ? ` · ${deploymentProgress(session.stage, copy)}` : ""}${session?.error ? ` · ${deploymentFailure(session.error, copy)}` : ""}`);
    if (!this.disconnectPending && (!personalSelected || showWorkerUpdateAction)) cloudflareSetting.addButton((button) => button
    ...
    if (personalSelected && showWorkerUpdateAction) {
      new Setting(containerEl)
